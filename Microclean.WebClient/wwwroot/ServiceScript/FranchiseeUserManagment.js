...
function addNewEmployeeByFranchisee() {
    $("#overlay1").show();
    if (CheckRequiredFields()) {
        ...
        datasave('Franchisee/createuserbyfranchisee', formData).then(data => {
            $("#overlay1").hide();
            if (data.data !== null) {
                ...
                window.location = "/Account/FranchiseeUserList";
            }
        });
    }
}

function getFranchiseeUserViewById(id) {
    $("#overlay1").show();
    ...
    $.ajax({
        ...
        success: function (result) {
            $("#overlay1").hide();
            sessionStorage.setItem('franchiseeUserData', JSON.stringify(result.data));
            window.location.href = '/Account/ViewFranchiseeUser';
        }
    });
}

function getFranchiseeUserView() {
    $("#overlay1").show();
    var obj = JSON.parse(sessionStorage.franchiseeUserData);
...
function getFranchiseeUserForEdit() {
    $("#overlay1").show();
    var obj = JSON.parse(sessionStorage.franchiseeUserData);
    console.log(obj);
...
function updateUserByFranchisee() {
    $("#overlay1").show();
    var formData = new FormData();
    ...
    datasave('Franchisee/updatefranchiseeuser', formData).then(data => {
        $("#overlay1").hide();
